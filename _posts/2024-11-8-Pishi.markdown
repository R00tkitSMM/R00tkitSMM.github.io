...
This blog post is the result of some weekend research, where I delved into Pishi, a static macOS kernel binary rewriting tool.
...
Calling imported functions is done through `__auth_stubs` and this stub will [clobber](https://en.wikipedia.org/wiki/Clobbering) `X16`, `X17` and `LR` registers.
...
before we continue I have to say that what is happening under the hook for libFuzzer is way more complicated than this
...
guessing one out of 0xffffffff possible numbers, narrowed down to 16 possible answers each time.
{% highlight cpp %}
if (input == 0xabad1dea) {
  /* terribly buggy code */
} else {
...
At this point we have setup everything, we just need to instrument `com.apple.driver.AppleAVD` by some and enjoy our coverage-guided fuzzing of AppleAVD.\
is't this cool?
{% highlight cpp %}
...
#### Another approach?
I was considering an alternative approach If everything had failed, the well known method of replacing each BB of a target KEXT with a [breakpoint](https://developer.arm.com/documentation/102120/0101/Debug-exceptions), then boot a `Boot Kext Collection` with modified [EL1 exception handler](https://github.com/apple-oss-distributions/xnu/blob/8d741a5de7ff4191bf97d57b9f54c2f6d4a15585/osfmk/arm64/sleh.c#L909) in XNU that would log and restore every trapped BBs.
