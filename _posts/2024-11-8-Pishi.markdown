...
This blog post is the result of some weekend research, where I delved into [Pishi](https://www.quora.com/What-does-Hapoo-%D9%87%D8%A7%D9%BE%D9%88-and-Pishi-%D9%BE%DB%8C%D8%B4%DB%8C-means-in-Persian), a static macOS kernel binary rewriting tool, which I presented 
at [POC2024](https://powerofcommunity.net/2024.htm).
...
before getting started I also would like to thank [Ivan Fratric](https://x.com/ifsecure), for his valuable feedbacks, and mentioning Pishi in his [Google Project Zero blog post](https://googleprojectzero.blogspot.com/2024/11/simple-macos-kernel-extension-fuzzing.html)
...
6. Restore the Link Register (LR).
...
We can use directives `.rept .endr` to spray memory(not for [exploitation](https://en.wikipedia.org/wiki/Heap_spraying) this time) with plenty of trampolines.
...
`bti` or [branch target identification](https://en.wikipedia.org/wiki/Indirect_branch_tracking) instructions similar to PAC are not part of the program's intended behavior; there are emited by compiler to protect against [Return-oriented programming](https://en.wikipedia.org/wiki/Return-oriented_programming).
